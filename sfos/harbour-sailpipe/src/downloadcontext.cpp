...
void DownloadContext::open(QString const& filename, QString const& mimetype, qlonglong length)
{
  ...
  //% "SailPipe download"
  m_transferId = m_transferClient->createDownloadEvent(qtTrId("sailpipe_transfer_engine-sailpipe_download"), QUrl(), QUrl(QString::fromLatin1("image://theme/harbour-sailpipe")), localFile, mimetype, length, callback);
