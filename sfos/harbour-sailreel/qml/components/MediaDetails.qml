    ...
    MouseArea {
        ...
        Column {
            ...
            Row {
                ...
                StatItem {
                    source: "image://sailpipe/icon-s-media-view?" + Theme.highlightColor
                    //% "N/A"
                    text: mediaInfo.viewCount >= 0 ? mediaInfo.viewCount : qsTrId("sailpipe_media_details-detail_views_not_applicable")
                }

                StatItem {
                    source: "image://theme/icon-s-like?" + Theme.highlightColor
                    //% "N/A"
                    text: mediaInfo.likeCount >= 0 ? mediaInfo.likeCount : qsTrId("sailpipe_media_details-detail_likes_not_applicable")
                }
            }

            KeyValue {
                //% "Uploader"
                key: qsTrId("sailpipe_media_details-detail_uploader")
                value: mediaInfo.uploaderName
            }

            KeyValue {
                //% "Upload date"
                key: qsTrId("sailpipe_media_details-detail_date")
                value: Format.formatDate(mediaInfo.uploadDate, Format.DateLong)
            }

            KeyValue {
                //% "Length"
                key: qsTrId("sailpipe_media_details-detail_length")
                value: Utils.lengthToTimeString(mediaInfo.length)
            }

            KeyValue {
                //% "Category"
                key: qsTrId("sailpipe_media_details-detail_category")
                value: mediaInfo.category
            }

            KeyValue {
                //% "Licence"
                key: qsTrId("sailpipe_media_details-detail_licence")
                value: mediaInfo.licence
            }

            KeyValue {
                //% "Description"
                key: qsTrId("sailpipe_media_details-detail_description")
