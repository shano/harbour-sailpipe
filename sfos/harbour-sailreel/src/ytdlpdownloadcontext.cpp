...
void YtDlpDownloadContext::start()
{
  ...
  qlonglong length = 0;
  //% "SailReel download"
  m_transferId = m_transferClient->createDownloadEvent(qtTrId("sailpipe_transfer_engine-sailpipe_download"), QUrl(), QUrl(QString::fromLatin1("image://theme/harbour-sailreel")), localFile, QString::fromLatin1(YTDLP_MIMETYPE), length, callback);
